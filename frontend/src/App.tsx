...
type View = { name: "albums" } | { name: "create" } | { name: "import" } | { name: "album"; albumId: number } | { name: "edit"; albumId: number } | { name: "rate"; albumId: number } | { name: "reconcile"; albumId: number };
function readView(): View { const reconcile = window.location.pathname.match(/^\/albums\/(\d+)\/reconcile$/); const edit = window.location.pathname.match(/^\/albums\/(\d+)\/edit$/); const rate = window.location.pathname.match(/^\/albums\/(\d+)\/rate$/); const album = window.location.pathname.match(/^\/albums\/(\d+)$/); return reconcile ? { name: "reconcile", albumId: Number(reconcile[1]) } : edit ? { name: "edit", albumId: Number(edit[1]) } : rate ? { name: "rate", albumId: Number(rate[1]) } : album ? { name: "album", albumId: Number(album[1]) } : window.location.pathname === "/albums/new" ? { name: "create" } : window.location.pathname === "/import" ? { name: "import" } : { name: "albums" }; }
export function App() { const [view, setView] = useState(readView); const navigate = (next: View) => { const path = next.name === "albums" ? "/" : next.name === "create" ? "/albums/new" : next.name === "import" ? "/import" : next.name === "album" ? `/albums/${next.albumId}` : next.name === "edit" ? `/albums/${next.albumId}/edit` : next.name === "reconcile" ? `/albums/${next.albumId}/reconcile` : `/albums/${next.albumId}/rate`; window.history.pushState({}, "", path); setView(next); };
  ...
  if (view.name === "album") return <AlbumPage albumId={view.albumId} onBack={() => navigate({ name: "albums" })} onRate={() => navigate({ name: "rate", albumId: view.albumId })} onReconcile={() => navigate({ name: "reconcile", albumId: view.albumId })} onEdit={() => navigate({ name: "edit", albumId: view.albumId })} />;

...
export function App() { const [view, setView] = useState(readView); const navigate = (next: View, replace = false) => { const path = next.name === "albums" ? "/" : next.name === "artists" ? "/artists" : next.name === "artist" ? `/artists/${next.artistId}` : next.name === "create" ? "/albums/new" : next.name === "import" ? "/import" : next.name === "album" ? `/albums/${next.albumId}` : next.name === "edit" ? `/albums/${next.albumId}/edit` : next.name === "reconcile" ? `/albums/${next.albumId}/reconcile` : `/albums/${next.albumId}/rate`; window.history[replace ? "replaceState" : "pushState"]({}, "", path); setView(next); };
  ...
  if (view.name === "reconcile") return <LegacyReconciliationPage albumId={view.albumId} onBack={() => navigate({ name: "album", albumId: view.albumId })} onReconciled={() => navigate({ name: "album", albumId: view.albumId }, true)} />;
  ...
  if (view.name === "album") return <AlbumPage albumId={view.albumId} onBack={() => navigate({ name: "albums" })} onRate={() => navigate({ name: "rate", albumId: view.albumId })} onReconcile={() => navigate({ name: "reconcile", albumId: view.albumId })} onEdit={() => navigate({ name: "edit", albumId: view.albumId })} onArtist={(artistId) => navigate({ name: "artist", artistId })} onYear={(year) => { window.history.pushState({}, "", `/?decade=${Math.floor(year / 10) * 10}&year=${year}`); setView({ name: "albums" }); }} />;

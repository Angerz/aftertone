...
export interface FavoriteSongEntry { id: number; global_position: number; track_id: number; track_title: string; artists: Artist[]; album_id: number; album_title: string; album_year: number | null; cover_url: string | null; disc_number: number; track_position: number; base_score: number; emotional_connection: number; replay_value: number; originality: number; genre: string | null; notes: string | null; final_score: number; rank_movement: "unchanged" | "up" | "down" | "new"; rank_delta: number; top_100_entered_at: string | null }

...
export interface DraftTrack { trackId: number; title: string; discNumber: number; position: number; score: string | null; include: boolean; notes: string }
...
export function emptyRatingDraft(album: Album): RatingDraft {
  return { tracks: album.tracks.map((track) => ({ trackId: track.id, title: track.title, discNumber: track.disc_number, position: track.position, score: null, include: true, notes: "" })), coherence: "5", coherenceNotes: "", emotion: "5", emotionNotes: "", albumNotes: "" };
}
  ...
  return {
    draft: {
      tracks: album.tracks.map((track) => {
        ...
        return snapshot ? { trackId: track.id, title: track.title, discNumber: track.disc_number, position: track.position, score: snapshot.score === null ? null : String(snapshot.score), include: snapshot.include_in_pre_rating, notes: snapshot.notes ?? "" } : { trackId: track.id, title: track.title, discNumber: track.disc_number, position: track.position, score: null, include: true, notes: "" };
      }),

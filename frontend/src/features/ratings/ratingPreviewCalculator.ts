 ...
 */
export interface PreviewTrack { score: number | null; includeInPreRating: boolean }
...
export function ratingPreviewCalculator(tracks: PreviewTrack[], coherence: number, emotion: number): RatingPreview {
  const included = tracks.filter((track): track is PreviewTrack & { score: number } => track.includeInPreRating && track.score !== null);

...
export function RevisionDetailPage({ album, revision, onBack }: { album: Album; revision: RatingRevisionDetail; onBack: () => void }) {
  return <main className="page editor revision-detail"><button className="back" onClick={onBack}>← Back to current rating</button><header className="album-header"><p className="eyebrow">Historical snapshot</p><h1>{album.title}</h1><p>Viewing revision from {formatRevisionDate(revision.created_at)} · {album.artist}</p></header>
    <div className="detail-layout"><section><h2>Tracks</h2>{revision.tracks.map((track) => <article className="snapshot-track" key={track.track_id}><h3><span>{String(track.position).padStart(2, "0")}</span>{track.title}</h3><p className="snapshot-score">Rating <strong>{track.score.toFixed(1)}</strong> · {track.include_in_pre_rating ? "Included in PRE" : "Excluded from PRE"}</p><p className="snapshot-copy">{track.notes || "No notes."}</p></article>)}</section>

...
const tracks = (...scores: Array<number | null>) => scores.map((score, index) => ({ track_id: index + 1, position: index + 1, title: `Track ${index + 1}`, score, include_in_pre_rating: true, notes: null }));

  ...
  return <section className="album-momentum" onMouseLeave={() => setActiveIndex(null)}>
    ...
    <div className="momentum-chart">
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Album momentum track score chart">
        ...
        {data.points.map((point, index) => <rect key={`hit-${point.position}`} x={Math.max(left, x(index) - band / 2)} y={top} width={Math.min(band, width - right - Math.max(left, x(index) - band / 2))} height={plotHeight + bottom} className="momentum-hit-area" aria-label={`${String(point.position).padStart(2, "0")} · ${point.title}: ${point.score === null ? "Unrated" : point.score.toFixed(1)}`} tabIndex={0} onMouseEnter={() => setActiveIndex(index)} onFocus={() => setActiveIndex(index)} />)}
        {data.points.map((point, index) => (index % labelEvery === 0 || index === data.points.length - 1) && <text key={point.position} x={x(index)} y={height - 10} textAnchor="middle" className={`momentum-label momentum-x-label${activeIndex === index ? " active" : ""}`}>{String(point.position).padStart(2, "0")}</text>)}
      </svg>
      {active && <div className="momentum-tooltip" style={{ left: `${tooltipLeft}%`, top: `${tooltipTop}%` }}><strong>{String(active.position).padStart(2, "0")}</strong><span>{active.title}</span><b>{active.score === null ? "Unrated" : active.score.toFixed(1)}</b></div>}
    </div>

...
import { getAlbumFacets, getAlbumRatingSummary, listAlbums } from "../../api/albums";
import type { Album, AlbumFacets, AlbumRatingSummary, PaginatedResponse } from "../../api/types";
...
import { getRatingTier } from "./ratingTier";
...
function LibraryRatingSummaryBlock({ summary, loading }: { summary: AlbumRatingSummary | null; loading: boolean }) {
  const tier = getRatingTier(summary?.average);
  return <aside className={`library-rating-summary ${tier ? `rating-${tier}` : "unrated"}`} aria-live="polite"><span>Average rating</span><strong>{loading ? "…" : formatLibraryAverage(summary?.average ?? null)}</strong><small>{loading ? "Loading…" : ratedProjectLabel(summary?.rated_count ?? 0)}</small></aside>;
}

export function AlbumsPage({ onCreate, onImport, onArtists, onTopSongs, onOpen }: { onCreate: () => void; onImport: () => void; onArtists: () => void; onTopSongs: () => void; onOpen: (id: number) => void }) {
  ...
  const [summary, setSummary] = useState<AlbumRatingSummary | null>(null); const [summaryLoading, setSummaryLoading] = useState(false);
  ...
  const showSummary = shouldShowLibraryRatingSummary(navigation.search, navigation.unrated);
  useEffect(() => { if (!showSummary) { setSummary(null); setSummaryLoading(false); return; } let active = true; setSummary(null); setSummaryLoading(true); getAlbumRatingSummary({ decade: navigation.decade, year: navigation.year, revisit: navigation.revisit, unrated: navigation.unrated }).then((value) => { if (active) setSummary(value); }).catch((cause: Error) => { if (active) setError(cause.message); }).finally(() => { if (active) setSummaryLoading(false); }); return () => { active = false; }; }, [navigation.decade, navigation.year, navigation.revisit, navigation.unrated, showSummary]);
  ...
  return <main className="page">
    <header className="masthead library-header"><div><p className="eyebrow">Aftertone</p><h1>Library</h1><p>Your collection of listened-to records.</p></div><div className="library-actions"><button className="text-button" onClick={onTopSongs}>Top 100</button><button className="text-button" onClick={onArtists}>Artists</button><button className="text-button" onClick={onImport}>Import legacy ratings</button><button onClick={onCreate}>+ Add album</button></div></header>
